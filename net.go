...
func (m *Memberlist) handleConn(conn *net.TCPConn) {
	...
	if msgType == userMsg {
		...
		if err != nil {
			m.logger.Printf("memberlist: Failed to read remote state: %s", err)
			return
		}

		if err := m.sendLocalState(conn, join); err != nil {
			m.logger.Printf("memberlist: Failed to push local state: %s", err)
			return
		}
		...
		m.logger.Printf("memberlist: Received invalid msgType (%d)", msgType)
	}
}
